...
class NN(nn.Module):
    ...
    def __init__(
        ...
        self.residuals = nn.ModuleDict()  # Initialize here (this was missing)
        ...
        if (start < end):
            if shape_start == shape_end:
                ...
                if len(shape_start) == len(shape_end):
                    ...
                    raise ValueError(
                        f"The lenghts of shape at layer {start} and {end} need to be equal to have a residual connection. Got {shape_start} and {shape_end}.")
        else:
        ...
        if not self.save:  # Regular forward pass
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i in self.residuals:
                    x = self.apply_residual(x, inputs_residuals, layer=i)
                if isinstance(layer,
                              (nn.Conv2d, nn.BatchNorm2d, nn.AvgPool2d, nn.AdaptiveAvgPool2d, nn.Linear, nn.Flatten)):
                    x = layer(x)
    ...
    def apply_residual(self, x: torch.Tensor, outputs: list[torch.Tensor], layer: int,
