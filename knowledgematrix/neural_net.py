...
class NN(nn.Module):
    ...
    def get_input_size(self) -> int:
        # input_shape may be a bare feature count (MNIST-1D and friends)
        # rather than a tuple.
    ...
    def freeze_features(self) -> None:
        # Puts requires_grad = False on the CONVOLUTIONAL layers only,
        # leaving the classifier trainable: the transfer-learning setup,
        # where a pretrained feature extractor is reused as-is and only the
        # head is fit to the new label set.
        for layer in self.layers:
